...
async function main() {
    const mcp = new McpServer({ name: "lore-mcp", version: "0.1.0" }, { instructions: serverInstructions });
    const tools = createTools();
    mcp.tool(tools.search_lore.name, tools.search_lore.description, {
        query: z.string().min(1),
        limit: z.number().int().positive().max(200).default(20).optional(),
        scope: z.string().regex(/^[A-Za-z0-9._-]+$/).optional(),
        baseUrl: z.string().url().optional(),
    }, async (args) => tools.search_lore.handler(args));
    mcp.tool(tools.get_message_raw.name, tools.get_message_raw.description, {
    ...
    mcp.tool(tools.get_thread_summary.name, tools.get_thread_summary.description, {
        url: z.string().url().optional(),
        messageId: z.string().optional(),
        scope: z.string().regex(/^[A-Za-z0-9._-]+$/).optional(),
        list: z.string().optional(),
    ...
    mcp.tool(tools.summarize_thread_llm.name, tools.summarize_thread_llm.description, {
        url: z.string().url().optional(),
        messageId: z.string().optional(),
        scope: z.string().regex(/^[A-Za-z0-9._-]+$/).optional(),
        list: z.string().optional(),
    ...
    mcp.tool(tools.get_patchset.name, tools.get_patchset.description, {
        url: z.string().url().optional(),
        messageId: z.string().optional(),
        scope: z.string().regex(/^[A-Za-z0-9._-]+$/).optional(),
        list: z.string().optional(),
    ...
    mcp.tool(tools.get_thread_mbox.name, tools.get_thread_mbox.description, {
        url: z.string().url().optional(),
        messageId: z.string().optional(),
        scope: z.string().regex(/^[A-Za-z0-9._-]+$/).optional(),
        list: z.string().optional(),
    ...
    mcp.tool(tools.list_scopes.name, tools.list_scopes.description, {
        baseUrl: z.string().url().optional(),
    }, async (args) => tools.list_scopes.handler(args));
    // Resource: scopes listing (JSON)
    mcp.resource("scopes", "mcp://lore-mcp/scopes", async () => {
