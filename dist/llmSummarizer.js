...
function messageToContext(m, opts) {
    ...
    if (blocks.length > 0) {
        ...
        for (const [file, v] of per) {
            ...
            ins += v.ins;
            del += v.del;
            filesArr.push({ file, ins: v.ins, del: v.del });
        }
...
function buildUserPrompt(threadTitle, participants, chunkIdx, totalChunks) {
...
export async function summarizeThreadLLM(messages, opts = {}) {
