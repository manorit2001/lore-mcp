 ...
 */
export function shouldBypassProxy(url, noProxyList) {
    ...
    try {
        ...
        for (const pattern of noProxyList) {
            ...
            if (port && `${hostname}:${port}` === cleanPattern)
                return true;
            ...
            if (isLocalCidrMatch(hostname, cleanPattern))
                return true;
        }
        return false;
    }
 ...
 */
function isLocalCidrMatch(hostname, pattern) {
    ...
    if (pattern.includes("/")) {
        ...
        if (isNaN(networkBits) || networkBits < 0 || networkBits > 32)
            return false;
        // Simple prefix matching for common cases
        const hostParts = hostname.split(".").map(Number);
        const netParts = network.split(".").map(Number);
        if (hostParts.length !== 4 || netParts.length !== 4)
            return false;
        if (hostParts.some(isNaN) || netParts.some(isNaN))
            return false;
 ...
 */
export function getProxyUrl(targetUrl, config) {
    if (!config.enabled)
        return undefined;
 ...
 */
export function createProxyFetch(proxyConfig) {
